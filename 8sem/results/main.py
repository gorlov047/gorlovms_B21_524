...
if __name__ == "__main__":
    with open("8sem/results/output/result.txt", "w") as file:
        for i in os.listdir("8sem/results/input"):
            if i == ".DS_Store":
                continue
            img = Image.open('8sem/results/input/'+i).convert('L')
            img.save("8sem/results/output/grayscale/"+i+"")
            semi = np.array(Image.open(os.path.join('8sem/results/output', 'grayscale', i)).convert('L'))
            contrast_grayscale = contrast(semi)
            Image.fromarray(contrast_grayscale).save("8sem/results/output/contrast/"+i+"")

            #grayscale  
            matrix1, k = haralick_matrix(semi, 1)
            Image.fromarray(matrix1/k * 255/np.max(matrix1/k)).convert('L').save('8sem/results/output/grayscale/' + "haralick_"+i)
            
            hist, bins = np.histogram(semi.flatten(), 256, [0, 256])
            plt.bar(np.arange(hist.size), hist)
            plt.savefig("8sem/results/output/grayscale/"+"histogram_"+i)
            plt.clf()
            

            #contrast
            matrix2, k = haralick_matrix(contrast_grayscale, 1)
            Image.fromarray(matrix2/k * 255/np.max(matrix2/k)).convert('L').save('8sem/results/output/contrast/' + "haralick_"+i)
            
            hist, bins = np.histogram(contrast_grayscale.flatten(), 256, [0, 256])
            plt.bar(np.arange(hist.size), hist)
            plt.savefig("8sem/results/output/contrast/"+"histogram_"+i)
            plt.clf()
